...
function MochaJUnitReporter(runner, options) {
  ...
  this._runner.on(
    'pass',
    function(test) {
      var suiteName = lastSuite()[0]._attr.name;
      lastSuite().push(this.getTestcaseData(test, suiteName));
    }.bind(this)
  );
  ...
  this._runner.on(
    ...
    function(test, err) {
      var suiteName = lastSuite()[0]._attr.name;
      lastSuite().push(this.getTestcaseData(test, suiteName, err));
    }.bind(this)
  );
  ...
  if (this._options.includePending) {
    ...
    this._runner.on(
      'pending',
      function(test) {
        var testcase = this.getTestcaseData(test, suiteName);
 ...
 * @returns {object}
 */
MochaJUnitReporter.prototype.getTestcaseData = function(test, suiteName, err) {
  ...
  var suiteName = stripAnsi(suiteName);
  var config = {
    testcase: [
      {
        _attr: {
          ...
          classname: flipClassAndName ? name : suiteName
        }
      }
    ]
  };
